...
const saveFunction = (get: Getter, set: Setter) => {
  ...
  fetch((window as any).vizarrApi.roisUrl, {
    ...
    .then(savedData => {
      console.log('Data received from server:', savedData);
      // Update the frontend's state with the authoritative data from the server
      // this gives newly drawn rois an ID that was generated by django and makes deleting individual rois possible without refresh
      set(roiCollectionAtom, savedData);
      console.log('ROIs saved and state updated with new ID!');
    })

    .catch(error => {
      console.error('Error while saving ROIs:', error);
    });
};
...
export const deleteSelectedRoiAtom = atom(
  ...
  async (get, set) => {
    ...
    if (!roiId) {
      ...
      const deleteUrl = `${(window as any).vizarrApi.roisUrl}${roiId}/`;
